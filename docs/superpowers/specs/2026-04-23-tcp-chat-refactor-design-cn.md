...
- `BuildAuthRequest(cmd, username, password string) string`：构造登录或注册请求。
- `BuildPublicMessage(content string) string`：构造客户端公聊消息。
- `BuildPrivateEnterRequest(target string) string`：构造进入私聊请求。
- `BuildPrivateMessage(target, content string) string`：构造客户端私聊消息。
- `BuildListRequest() string`：构造在线列表请求。
- `BuildQuitRequest() string`：构造客户端退出请求。
- `BuildAuthOK(action string) string`：构造认证成功响应。
- `BuildAuthErr(code string) string`：构造认证失败响应。
- `BuildSystemInfo(message string) string`：构造系统提示消息。
- `BuildSystemErr(message string) string`：构造系统错误消息。
- `BuildPublicBroadcast(sender, content string) string`：构造服务端公聊广播消息。
- `BuildPrivateInbound(sender, content string) string`：构造发给私聊接收者的消息。
- `BuildPrivateAck(target, content string) string`：构造发给私聊发送者的回执。
- `BuildPrivateEnterOK(target string) string`：构造进入私聊成功响应。
- `BuildPrivateEnterErr(code string) string`：构造进入私聊失败响应。
- `BuildUserList(users []string) string`：构造在线用户列表响应。
- `BuildShutdown(message string) string`：构造服务端关闭通知。
